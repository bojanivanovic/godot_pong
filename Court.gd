...
func _process(_delta):
	# set the countdown lable to seconds left on the timer at the start and between scores
...
func score_achieved(player):
	...
	if player == "playerLeft":
		# increase score and update to score lable to reflect the added point
		playerScore += 1
		$PlayerLeftScore.text = str(playerScore)
...
# resets everything to initial positions on the screen
